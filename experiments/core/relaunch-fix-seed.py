    ...
    def relaunch(self, top_dir):
        ...
        for exp_dir in glob(os.path.join(top_dir, "*")):
            ...
            if not self.test:
                ...
                # Fix seed
                expfile = open("experiment-script_000.sh", "r")
                expcontent = expfile.read()
                expfile.close()
                expfile = open("experiment-script_000.sh", "w")
                expcontent = re.sub(r"--seed [^ ]+ ", "--seed %d " % (random.randint(0, 30000000)), expcontent)
                expfile.write(expcontent)
                expfile.close()
